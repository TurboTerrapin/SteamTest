...
public class DirectionalShifter : NetworkBehaviour, IControllable, IPowerable, IIKTargetable
{
    ...
    private static string INFO_MESSAGE = "Handles the direction the ship travels at impulse speed.";
    ...
    private void transmitDirectionalShifterRPC(float sp, bool reverse)
    {
        shift_percentage = sp;
        in_reverse = reverse;
        ReferenceAssistor.Instance.spaceship.GetComponent<ShipMovement>().ShiftDirection(in_reverse);
        displayAdjustment();
    }
}
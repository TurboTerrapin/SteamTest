...
public class BlackAndWhite : NetworkBehaviour, IScenario, IComputerRegulatorSusceptible
{
    ...
    private int[] emitter_radiation_options = new int[6] { -1, -1, -1, -1, -1, -1 }; //corresponds to A, B, C, D, E, and F mesh options
    ...
    public void initiateScenario()
    {
        ...
        for (int i = 0; i < 6; i++)
        {
            ...
            emitter_radiation_options[i] = current_option;
        }
        ...
        initializeGateAppearanceRPC(DataConverter.arrayToString(emitter_radiation_options), DataConverter.arrayToString(vertical_light_colors), DataConverter.arrayToString(horizontal_light_colors));
    }
    ...
    private void initializeGateAppearanceRPC(string radiation_locations, string vert_colors, string horiz_colors)
    {
        ...
        emitter_radiation_options = DataConverter.stringToArray(radiation_locations);
        for (int i = 0; i < 6; i++)
        {
            radiation_emitters[i].transform.GetChild(0).GetComponent<MeshFilter>().mesh = radiation_options[emitter_radiation_options[i]];
        }
